...
__device__
inline
void solve_tridiag(int jc, int nlevels, int blockNo, mdspan_2d<T> a,
                   mdspan_2d<T> b, mdspan_2d<T> c, mdspan_2d<T> d,
    ...
    for (int level = 1; level < nlevels+1; level++) {
        T m = b(level, jc) - cp(level-1, jc) * a(level, jc);
        T fxa = 1.0 / m;
...
void integrate(int jc, int blockNo,
    ...
    if (p_constant_tke.only_tke) {
        for (int level = 0; level < dolic+1; level++) {
            p_cvmix.tke(blockNo, level, jc) = max(p_cvmix.tke(blockNo, level, jc), p_constant_tke.tke_min); // NOLINT
        }
    }
