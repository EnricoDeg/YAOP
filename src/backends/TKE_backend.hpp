 ...
 protected:
    ...
    */
    ...
    void fill_struct_memview(t_patch_view<T, memview, dext> *p_patch_view,
        ...
        p_patch_view->prism_center_dist_c = memview_policy<T>::memview(p_patch->prism_center_dist_c,
                                                                    nblocks, nlevs+1, nproma);
        p_patch_view->inv_prism_center_dist_c = memview_policy<T>::memview(p_patch->inv_prism_center_dist_c,
                                                                        nblocks, nlevs+1, nproma);
        p_patch_view->prism_thick_c = memview_policy<T>::memview(p_patch->prism_thick_c, nblocks, nlevs, nproma);
        p_patch_view->dolic_c = memview_policy<int>::memview(p_patch->dolic_c, nblocks, nproma);
        p_patch_view->dolic_e = memview_policy<int>::memview(p_patch->dolic_e, nblocks, nproma);
        p_patch_view->zlev_i = memview_policy<T>::memview(p_patch->zlev_i, nlevs);
        p_patch_view->wet_c = memview_policy<T>::memview(p_patch->wet_c, nblocks, nlevs, nproma);
        p_patch_view->edges_cell_idx = memview_policy<int>::memview(p_patch->edges_cell_idx, 2, nlevs, nproma);
        p_patch_view->edges_cell_blk = memview_policy<int>::memview(p_patch->edges_cell_blk, 2, nlevs, nproma);
    }
    ...
    */
    template <class T,
              ...
              template <class, size_t> class dext,
              template <class> class memview_policy>
    void fill_struct_memview(t_sea_ice_view<T, memview, dext> *p_sea_ice_view, t_sea_ice<T> *p_sea_ice,
    ...
    */
    template <class T,
              ...
              template <class, size_t> class dext,
              template <class> class memview_policy>
    void fill_struct_memview(t_atmos_for_ocean_view<T, memview, dext> *p_as_view, t_atmos_for_ocean<T> *p_as,
                             int nblocks, int nproma) {
        p_as_view->fu10 = memview_policy<T>::memview(p_as->fu10, nblocks, nproma);
    }
    ...
    */
    ...
    void fill_struct_memview(t_ocean_state_view<T, memview, dext> *ocean_state_view,
                             t_ocean_state<T> *ocean_state, int nblocks, int nlevs, int nproma) {
        ...
        ocean_state_view->stretch_c = memview_policy<T>::memview(ocean_state->stretch_c, nblocks, nproma);
        ocean_state_view->eta_c = memview_policy<T>::memview(ocean_state->eta_c, nblocks, nproma);
        ocean_state_view->p_vn_x1 = memview_policy<T>::memview(ocean_state->p_vn_x1, nblocks, nlevs, nproma);
        ocean_state_view->p_vn_x2 = memview_policy<T>::memview(ocean_state->p_vn_x2, nblocks, nlevs, nproma);
        ocean_state_view->p_vn_x3 = memview_policy<T>::memview(ocean_state->p_vn_x3, nblocks, nlevs, nproma);
    }
    ...
    */
    template <class T,
              ...
              template <class, size_t> class dext,
              template <class> class memview_policy>
    void fill_struct_memview(t_cvmix_view<T, memview, dext> *p_cvmix_view, t_cvmix<T> *p_cvmix,
        ...
        p_cvmix_view->hlc = memview_policy<T>::memview(p_cvmix->hlc, nblocks, nproma);
        p_cvmix_view->wlc = memview_policy<T>::memview(p_cvmix->wlc, nblocks, nlevs+1, nproma);
        p_cvmix_view->u_stokes = memview_policy<T>::memview(p_cvmix->u_stokes, nblocks, nproma);
        p_cvmix_view->a_veloc_v = memview_policy<T>::memview(p_cvmix->a_veloc_v, nblocks, nlevs+1, nproma);
        p_cvmix_view->a_temp_v = memview_policy<T>::memview(p_cvmix->a_temp_v, nblocks, nlevs+1, nproma);
        p_cvmix_view->a_salt_v = memview_policy<T>::memview(p_cvmix->a_salt_v, nblocks, nlevs+1, nproma);
        p_cvmix_view->iwe_Tdis = memview_policy<T>::memview(p_cvmix->iwe_Tdis, nblocks, nlevs+1, nproma);
        p_cvmix_view->cvmix_dummy_1 = memview_policy<T>::memview(p_cvmix->cvmix_dummy_1, nblocks, nlevs+1, nproma);
        p_cvmix_view->cvmix_dummy_2 = memview_policy<T>::memview(p_cvmix->cvmix_dummy_2, nblocks, nlevs+1, nproma);
        p_cvmix_view->cvmix_dummy_3 = memview_policy<T>::memview(p_cvmix->cvmix_dummy_3, nblocks, nlevs+1, nproma);
        p_cvmix_view->tke_Tbpr = memview_policy<T>::memview(p_cvmix->tke_Tbpr, nblocks, nlevs+1, nproma);
        p_cvmix_view->tke_Tspr = memview_policy<T>::memview(p_cvmix->tke_Tspr, nblocks, nlevs+1, nproma);
        p_cvmix_view->tke_Tdif = memview_policy<T>::memview(p_cvmix->tke_Tdif, nblocks, nlevs+1, nproma);
        p_cvmix_view->tke_Tdis = memview_policy<T>::memview(p_cvmix->tke_Tdis, nblocks, nlevs+1, nproma);
        p_cvmix_view->tke_Twin = memview_policy<T>::memview(p_cvmix->tke_Twin, nblocks, nlevs+1, nproma);
        p_cvmix_view->tke_Tiwf = memview_policy<T>::memview(p_cvmix->tke_Tiwf, nblocks, nlevs+1, nproma);
        p_cvmix_view->tke_Tbck = memview_policy<T>::memview(p_cvmix->tke_Tbck, nblocks, nlevs+1, nproma);
        p_cvmix_view->tke_Ttot = memview_policy<T>::memview(p_cvmix->tke_Ttot, nblocks, nlevs+1, nproma);
        p_cvmix_view->tke_Lmix = memview_policy<T>::memview(p_cvmix->tke_Lmix, nblocks, nlevs+1, nproma);
        p_cvmix_view->tke_Pr = memview_policy<T>::memview(p_cvmix->tke_Pr, nblocks, nlevs+1, nproma);
    }
    ...
    */
    ...
    void internal_fields_malloc(t_tke_internal_view<T, memview, dext> *p_internal_view) {
        ...
        p_internal_view->dzw_stretched = this->memview_malloc<T, memview, dext, memview_policy>
                                               (m_dzw_stretched, p_constant.nlevs, p_constant.nproma);
        p_internal_view->dzt_stretched = this->memview_malloc<T, memview, dext, memview_policy>
                                               (m_dzt_stretched, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->tke_Av = this->memview_malloc<T, memview, dext, memview_policy>
                                        (m_tke_Av, p_constant.nblocks, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->tke_kv = this->memview_malloc<T, memview, dext, memview_policy>
                                        (m_tke_kv, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->Nsqr = this->memview_malloc<T, memview, dext, memview_policy>
                                      (m_Nsqr, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->Ssqr = this->memview_malloc<T, memview, dext, memview_policy>
                                      (m_Ssqr, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->a_dif = this->memview_malloc<T, memview, dext, memview_policy>
                                       (m_a_dif, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->b_dif = this->memview_malloc<T, memview, dext, memview_policy>
                                       (m_b_dif, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->c_dif = this->memview_malloc<T, memview, dext, memview_policy>
                                       (m_c_dif, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->a_tri = this->memview_malloc<T, memview, dext, memview_policy>
                                       (m_a_tri, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->b_tri = this->memview_malloc<T, memview, dext, memview_policy>
                                       (m_b_tri, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->c_tri = this->memview_malloc<T, memview, dext, memview_policy>
                                       (m_c_tri, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->d_tri = this->memview_malloc<T, memview, dext, memview_policy>
                                       (m_d_tri, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->sqrttke = this->memview_malloc<T, memview, dext, memview_policy>
                                         (m_sqrttke, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->forc = this->memview_malloc<T, memview, dext, memview_policy>
                                      (m_forc, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->ke = this->memview_malloc<T, memview, dext, memview_policy>
                                    (m_ke, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->cp = this->memview_malloc<T, memview, dext, memview_policy>
                                    (m_cp, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->dp = this->memview_malloc<T, memview, dext, memview_policy>
                                    (m_dp, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->tke_upd = this->memview_malloc<T, memview, dext, memview_policy>
                                         (m_tke_upd, p_constant.nlevs+1, p_constant.nproma);
        p_internal_view->tke_unrest = this->memview_malloc<T, memview, dext, memview_policy>
    ...
    */
    ...
    void internal_fields_free() {
        this->memview_free<T, memview_policy>(m_tke_old);
        this->memview_free<T, memview_policy>(m_dzw_stretched);
        this->memview_free<T, memview_policy>(m_dzt_stretched);
        this->memview_free<T, memview_policy>(m_tke_old);
        this->memview_free<T, memview_policy>(m_tke_Av);
        this->memview_free<T, memview_policy>(m_tke_kv);
        this->memview_free<T, memview_policy>(m_Nsqr);
        this->memview_free<T, memview_policy>(m_Ssqr);
        this->memview_free<T, memview_policy>(m_a_dif);
        this->memview_free<T, memview_policy>(m_b_dif);
        this->memview_free<T, memview_policy>(m_c_dif);
        this->memview_free<T, memview_policy>(m_a_tri);
        this->memview_free<T, memview_policy>(m_b_tri);
        this->memview_free<T, memview_policy>(m_c_tri);
        this->memview_free<T, memview_policy>(m_d_tri);
        this->memview_free<T, memview_policy>(m_sqrttke);
        this->memview_free<T, memview_policy>(m_forc);
        this->memview_free<T, memview_policy>(m_ke);
        this->memview_free<T, memview_policy>(m_cp);
        this->memview_free<T, memview_policy>(m_dp);
        this->memview_free<T, memview_policy>(m_tke_upd);
        this->memview_free<T, memview_policy>(m_tke_unrest);
    }
