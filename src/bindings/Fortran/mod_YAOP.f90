...
module mod_YAOP
    ...
    interface yaop_init_f
        module procedure yaop_init_dp_f
        module procedure yaop_init_sp_f
    end interface

    interface yaop_calc_tke_f
        module procedure yaop_calc_tke_dp_f
        module procedure yaop_calc_tke_sp_f
    end interface
    ...
    !> YAOP initialization.
    !!
    !! It calls the YAOP_Init C function.
    subroutine yaop_init_dp_f(nproma, nlevs, nblocks, vert_mix_type, vmix_idemix_tke, &
        ...
        interface
            subroutine yaop_init_c(nproma, nlevs, nblocks, vert_mix_type, vmix_idemix_tke, &
                                  vert_cor_type, dtime, OceanReferenceDensity, grav, &
                                  l_lc, clc, ReferencePressureIndbars, pi) bind(C, name="YAOP_Init")
                use iso_c_binding
                implicit none
            ...
            end subroutine yaop_init_c
        end interface

        CALL yaop_init_c(nproma, nlevs, nblocks, vert_mix_type, vmix_idemix_tke, &
                        vert_cor_type, dtime, OceanReferenceDensity, grav, &
                        l_lc, clc, ReferencePressureIndbars, pi)
    end subroutine yaop_init_dp_f

    subroutine yaop_init_sp_f(nproma, nlevs, nblocks, vert_mix_type, vmix_idemix_tke, &
                          vert_cor_type, dtime, OceanReferenceDensity, grav, &
                          l_lc, clc, ReferencePressureIndbars, pi)
        implicit none
        integer, intent(in)  :: nproma
        ...
        integer, intent(in)  :: vmix_idemix_tke
        integer, intent(in)  :: vert_cor_type
        real(sp), intent(in) :: dtime
        real(sp), intent(in) :: OceanReferenceDensity
        real(sp), intent(in) :: grav
        integer, intent(in)  :: l_lc
        real(sp), intent(in) :: clc
        real(sp), intent(in) :: ReferencePressureIndbars
        real(sp), intent(in) :: pi

        interface
            subroutine yaop_init_c_sp(nproma, nlevs, nblocks, vert_mix_type, vmix_idemix_tke, &
                                  vert_cor_type, dtime, OceanReferenceDensity, grav, &
                                  l_lc, clc, ReferencePressureIndbars, pi) bind(C, name="YAOP_Init_sp")
                use iso_c_binding
                implicit none
                ...
                integer(c_int), value :: vmix_idemix_tke
                integer(c_int), value :: vert_cor_type
                real(c_float), value :: dtime
                real(c_float), value :: OceanReferenceDensity
                real(c_float), value :: grav
                integer(c_int), value :: l_lc
                real(c_float), value :: clc
                real(c_float), value :: ReferencePressureIndbars
                real(c_float), value :: pi
            end subroutine yaop_init_c_sp
        end interface

        CALL yaop_init_c_sp(nproma, nlevs, nblocks, vert_mix_type, vmix_idemix_tke, &
                        vert_cor_type, dtime, OceanReferenceDensity, grav, &
                        l_lc, clc, ReferencePressureIndbars, pi)
    end subroutine yaop_init_sp_f

    !> YAOP finalization.
    !!
    !! It calls the YAOP_Finalize C function.
    subroutine yaop_finalize_f()
        implicit none

        interface
            subroutine yaop_finalize_c() bind(C, name="YAOP_Finalize")
                use iso_c_binding
                implicit none

            end subroutine yaop_finalize_c
        end interface

        CALL yaop_finalize_c()
    end subroutine yaop_finalize_f

    !> YAOP time loop calculation.
    !!
    !! It calls the YAOP_Calc_tke C function.
    subroutine yaop_calc_tke_dp_f(depth_CellInterface, prism_center_dist_c, &
        ...
        interface
            subroutine yaop_calc_tke_c(depth_CellInterface_c, prism_center_dist_c_c, &
                                       ...
                                       cells_end_index_c) bind(C, name="YAOP_Calc_tke")
                use iso_c_binding
            ...
            end subroutine yaop_calc_tke_c
        end interface
        ...
        CALL yaop_calc_tke_c(depth_CellInterface_ptr, prism_center_dist_c_ptr, &
                             inv_prism_center_dist_c_ptr, prism_thick_c_ptr, &
                             dolic_c_ptr, dolic_e_ptr, zlev_i_ptr, wet_c_ptr, &
                             edges_cell_idx_ptr, edges_cell_blk_ptr, &
                             temp_ptr, salt_ptr, stretch_c_ptr, eta_c_ptr, &
    ...
    end subroutine yaop_calc_tke_dp_f

    subroutine yaop_calc_tke_sp_f(depth_CellInterface, prism_center_dist_c, &
                               inv_prism_center_dist_c, prism_thick_c, &
                               dolic_c, dolic_e, zlev_i, wet_c, &
                               edges_cell_idx, edges_cell_blk, &
                               temp, salt, stretch_c, eta_c, &
        ...
        real(c_float), intent(in),    TARGET :: depth_CellInterface(:,:,:)
        real(c_float), intent(in),    TARGET :: prism_center_dist_c(:,:,:)
        real(c_float), intent(in),    TARGET :: inv_prism_center_dist_c(:,:,:)
        real(c_float), intent(in),    TARGET :: prism_thick_c( :,:,:)
        integer, intent(in),           TARGET :: dolic_c(:,:)
        integer, intent(in),           TARGET :: dolic_e(:,:)
        real(c_float), intent(in),    TARGET :: zlev_i(:)
        real(c_float), intent(in),    TARGET :: wet_c(:,:,:)
        integer, intent(in),           TARGET :: edges_cell_idx(:,:,:)
        integer, intent(in),           TARGET :: edges_cell_blk(:,:,:)
        real(c_float), intent(in),    TARGET :: temp(:,:,:)
        real(c_float), intent(in),    TARGET :: salt(:,:,:)
        real(c_float), intent(in),    TARGET :: stretch_c(:,:)
        real(c_float), intent(in),    TARGET :: eta_c(:,:)
        real(c_float), intent(in),    TARGET :: p_vn_x1(:,:,:)
        real(c_float), intent(in),    TARGET :: p_vn_x2(:,:,:)
        real(c_float), intent(in),    TARGET :: p_vn_x3(:,:,:)
        real(c_float), intent(inout), TARGET :: tke(:,:,:)
        real(c_float), intent(in),    TARGET :: tke_plc_in(:,:,:)
        real(c_float), intent(in),    TARGET :: hlc_in(:,:)
        real(c_float), intent(in),    TARGET :: wlc_in(:,:,:)
        real(c_float), intent(in),    TARGET :: u_stokes_in(:,:)
        real(c_float), intent(inout), TARGET :: a_veloc_v(:,:,:)
        real(c_float), intent(inout), TARGET :: a_temp_v(:,:,:)
        real(c_float), intent(inout), TARGET :: a_salt_v(:,:,:)
        real(c_float), intent(in),    TARGET :: iwe_Tdis(:,:,:)
        real(c_float), intent(inout), TARGET :: cvmix_dummy_1(:,:,:)
        real(c_float), intent(inout), TARGET :: cvmix_dummy_2(:,:,:)
        real(c_float), intent(inout), TARGET :: cvmix_dummy_3(:,:,:)
        real(c_float), intent(inout), TARGET :: tke_Tbpr(:,:,:)
        real(c_float), intent(inout), TARGET :: tke_Tspr(:,:,:)
        real(c_float), intent(inout), TARGET :: tke_Tdif(:,:,:)
        real(c_float), intent(inout), TARGET :: tke_Tdis(:,:,:)
        real(c_float), intent(inout), TARGET :: tke_Twin(:,:,:)
        real(c_float), intent(inout), TARGET :: tke_Tiwf(:,:,:)
        real(c_float), intent(inout), TARGET :: tke_Tbck(:,:,:)
        real(c_float), intent(inout), TARGET :: tke_Ttot(:,:,:)
        real(c_float), intent(inout), TARGET :: tke_Lmix(:,:,:)
        real(c_float), intent(inout), TARGET :: tke_Pr(:,:,:)
        real(c_float), intent(in),    TARGET :: stress_xw(:,:)
        real(c_float), intent(in),    TARGET :: stress_yw(:,:)
        real(c_float), intent(in),    TARGET :: fu10(:,:)
        real(c_float), intent(in),    TARGET :: concsum(:,:)
        integer, intent(in)                    :: edges_block_size
        integer, intent(in)                    :: edges_start_block
        integer, intent(in)                    :: edges_end_block
        integer, intent(in)                    :: edges_start_index
        ...
        type(c_ptr) :: depth_CellInterface_ptr, prism_center_dist_c_ptr, &
                       ...
                       stress_yw_ptr, fu10_ptr, concsum_ptr
        interface
            subroutine yaop_calc_tke_sp_c(depth_CellInterface_c, prism_center_dist_c_c, &
                                       inv_prism_center_dist_c_c, prism_thick_c_c, &
                                       dolic_c_c, dolic_e_c, zlev_i_c, wet_c_c, &
                                       edges_cell_idx_c, edges_cell_blk_c, &
                                       temp_c, salt_c, stretch_c_c, eta_c_c, &
                                       ...
                                       edges_start_index_c, edges_end_index_c, cells_block_size_c, &
                                       cells_start_block_c, cells_end_block_c, cells_start_index_c, &
                                       cells_end_index_c) bind(C, name="YAOP_Calc_tke_sp")
                use iso_c_binding
            ...
            end subroutine yaop_calc_tke_sp_c
        end interface
        ...
        CALL yaop_calc_tke_sp_c(depth_CellInterface_ptr, prism_center_dist_c_ptr, &
                             inv_prism_center_dist_c_ptr, prism_thick_c_ptr, &
                             dolic_c_ptr, dolic_e_ptr, zlev_i_ptr, wet_c_ptr, &
                             edges_cell_idx_ptr, edges_cell_blk_ptr, &
                             temp_ptr, salt_ptr, stretch_c_ptr, eta_c_ptr, &
    ...
    end subroutine yaop_calc_tke_sp_f
